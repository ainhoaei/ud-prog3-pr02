...
public class JUnitCoche {
	...
	public void testFuerzaAceleracionAtras() {
		...
		for (int i=0;i<tablaVel.length;i++) {
			...
			assertEquals( "Velocidad " + tablaVel[i], tablaFuerza[i]*coche.FuerzaAtras, coche.FuerzaAceleraionAtras(), 0.0000001 );
		}
	}
	...
	public void testFuerzaAceleracionAdelante() {
		...
		for (int i=0;i<tablaVel.length;i++) {
			coche.setVelocidad( tablaVel[i] );
			assertEquals( "Velocidad " + tablaVel[i], tablaFuerza[i]*coche.FuerzaAdelante, coche.fuerzaAceleracionAdelante(), 0.0000001 );
		}
	}
